...
%for i=1:num;
    ...
    Q(i,1)=metricMI(im1,im2,fused,1);
    
    % Tsallis entropy $Q_{TE}$
    Q(i,2)=metricMI(im1,im2,fused,3);

    % Wang - NCIE $Q_{NCIE}$
    Q(i,3)=metricWang(im1,im2,fused);
    
    % Xydeas $Q_G$
    Q(i,4)=metricXydeas(im1,im2,fused);
    
    % PWW $Q_M$
    Q(i,5)=metricPWW(im1,im2,fused);
    
    %Yufeng Zheng (spatial frequency) $Q_{SF}$
    Q(i,6)=metricZheng(im1,im2,fused);
    
    % Zhao (phase congrency) $Q_P$
    Q(i,7)=metricZhao(im1,im2,fused);
    
    % Piella  (need to select only one) $Q_S$
    % Q(i,8)=index_fusion(im1,im2,fused{i});
    Q(i,8)=metricPeilla(im1,im2,fused,1);
    
    % Cvejie $Q_C$
    Q(i,9)=metricCvejic(im1,im2,fused,2);
    
    % Yang $Q_Y$
    Q(i,10)=metricYang(im1,im2, fused);
    
    % Chen-Varshney $Q_{CV}$
    Q(i,11)=metricChen(im1,im2,fused);
      
    % Chen-Blum $Q_{CB}$
    Q(i,12)=metricChenBlum(im1,im2,fused);
